...
async function handleContactsRequest(req, res) {
    ...
    if (req.url == "/api/fav") {
        ...
        if (req.url == "/api/categorie") {
            switch (req.method) {
                case "GET":
                    return response(res, 200, JSON.stringify(GetCategorie(contactsRepository)));
            }
        }

        if (req.url.includes("/api/categorie/")) {
            switch (req.method) {
                case "GET":
                    let categorie = req.url.substring(req.url.lastIndexOf("/") + 1, req.url.length);

                    let favorisArray = [];
                    let favoris = contactsRepository.getAll();
                    for (let fav of favoris) {
                        if (fav.Email == categorie) {
                            favorisArray.push(fav);
                        }
                    }
                    return response(res, 200, JSON.stringify(favorisArray));
            }
...
function GetCategorie(contactsRepository) {
    let favoris = contactsRepository.getAll();
    let categorie = { "categorie": [] };
    for (let fav of favoris) {
        if (!categorie.categorie.includes(fav.Email)) {
            categorie.categorie.push(fav.Email);
        }
    }
    return categorie;
}
